...
module overmind::split_or_steal {
    ...
    fun init_module(account: &signer) {
        ...
        if (!exists<State>(account_addr)) {
            move_to(account, State {
                    ...
                    cap: signer_cap,
                    create_game_events: account::new_event_handle(&resource_account),
                    submit_decision_events: account::new_event_handle(&resource_account),
                    reveal_decision_events: account::new_event_handle(&resource_account),
                    conclude_game_events: account::new_event_handle(&resource_account),
                    release_funds_after_expiration_events: account::new_event_handle(&resource_account)
            });
        }
    }
    ...
    */
    public entry fun create_game(
        ...
        let curr_game_id = get_next_game_id(&mut state.next_game_id);

        let player_one = PlayerData {
        ...
        let new_game = Game {
            ...
            expiration_timestamp_in_seconds: EXPIRATION_TIME_IN_SECONDS + timestamp::now_seconds()
        };

        simple_map::add(&mut state.games, curr_game_id, new_game);

        let resource_addr = account::get_signer_capability_address(&state.cap);

        coin::transfer<AptosCoin>(account, resource_addr, prize_pool_amount);
    ...
    */
    public entry fun submit_decision(
        ...
        check_if_player_participates_in_the_game(player, simple_map::borrow(&state.games, &game_id));

        let player_addr = signer::address_of(player);
        check_if_player_does_not_have_a_decision_submitted(simple_map::borrow(&state.games, &game_id), player_addr);

        let game = simple_map::borrow_mut(&mut state.games, &game_id);

        if (player_addr == game.player_one.player_address) {
            let player_one_decision_hash = &mut game.player_one.decision_hash;
            let player_one_salt_hash = &mut game.player_one.salt_hash;

            *player_one_decision_hash = option::some(decision_hash);
            *player_one_salt_hash = option::some(salt_hash);
        } else {
            let player_two_decision_hash = &mut game.player_two.decision_hash;
            let player_two_salt_hash = &mut game.player_two.salt_hash;

            *player_two_decision_hash = option::some(decision_hash);
            *player_two_salt_hash = option::some(salt_hash);
        };
    ...
    */
    public entry fun reveal_decision(
        ...
        check_if_both_players_have_a_decision_submitted(simple_map::borrow(&state.games, &game_id));

        let player_addr = signer::address_of(player);

        // sneaky way to avoid borrow checker
        {
            let game = simple_map::borrow_mut(&mut state.games, &game_id); //ref type alr

            let player = if (game.player_one.player_address == player_addr) {
                &mut game.player_one
            } else {
                &mut game.player_two
            };

            let player_decision = make_decision(player, &salt);

            event::emit_event(&mut state.reveal_decision_events, RevealDecisionEvent {
                game_id: game_id,
                player_address: player_addr,
                decision: player_decision, // player's decision
                event_creation_timestamp_in_seconds: timestamp::now_seconds()
            });
        };

        let game = *simple_map::borrow(&state.games, &game_id); // copy the reference such that the game in the map can be removed
        ...
        if (player_one_decision == DECISION_SPLIT && player_two_decision == DECISION_SPLIT) {
            // resource account & resource prize_pool_amount
            coin::transfer<AptosCoin>(&resource_signer, game.player_one.player_address, prize_pool_amount / 2);
            coin::transfer<AptosCoin>(&resource_signer, game.player_two.player_address, prize_pool_amount / 2);
        };
        ...
        if (player_one_decision == DECISION_STEAL && player_two_decision == DECISION_SPLIT) {
            coin::transfer<AptosCoin>(&resource_signer, game.player_one.player_address, prize_pool_amount);
        };

        if (player_one_decision == DECISION_SPLIT && player_two_decision == DECISION_STEAL) {
        ...
        // //          of the contract
        if (player_one_decision == DECISION_STEAL && player_two_decision == DECISION_STEAL) {
    ...
    */
    fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        // split decision hash
        let split_decision_bytes = bcs::to_bytes(&DECISION_SPLIT);
        vector::append(&mut split_decision_bytes, *string::bytes(salt));
        let split_decision_hash = hash::sha3_256(split_decision_bytes);

        // steal decision hash
        let steal_decision_bytes = bcs::to_bytes(&DECISION_STEAL);
        vector::append(&mut steal_decision_bytes, *string::bytes(salt));
    ...
    */
    ...
    fun check_if_account_has_enough_apt_coins(account: &signer, amount: u64) {
        let account_addr = signer::address_of(account);
        let coin_balance = coin::balance<AptosCoin>(account_addr);

        assert!(coin_balance >= amount, ESignerHasInsufficientAptBalance);
    }
    ...
    fun check_if_player_participates_in_the_game(player: &signer, game: &Game) {
        ...
        let player_one_addr = game.player_one.player_address;
        let player_two_addr = game.player_two.player_address;

        assert!(player_addr == player_one_addr || player_addr == player_two_addr, EPlayerDoesNotParticipateInTheGame);
    }

    fun check_if_both_players_have_a_decision_submitted(game: &Game) {
        assert!(option::is_some(&game.player_one.decision_hash), EBothPlayersDoNotHaveDecisionsSubmitted);
        assert!(option::is_some(&game.player_two.decision_hash), EBothPlayersDoNotHaveDecisionsSubmitted);
    }
    ...
    fun check_if_player_does_not_have_a_decision_submitted(game: &Game, player_address: address) {
        ...
        let player = if (player_address == player_one.player_address) {
            player_one
        } else {
            player_two
        };

        assert!(option::is_none(&player.decision_hash), EPlayerHasDecisionSubmitted);
        assert!(player_address == player_one.player_address || player_address == player_two.player_address, EPlayerDoesNotParticipateInTheGame);
    }
